...
ARCHIVE_FILE="/tmp/mongodb-backup-$(date +%Y%m%d).tgz"
...
# Create backup directory and dump MongoDB
mkdir -p $BACKUP_DIR
...
# Upload backup to S3
aws s3 cp $ARCHIVE_FILE s3://$BUCKET_NAME/ --region $REGION
...
# Close HTML without "All rights reserved"
cat <<EOL >> $OUTPUT_FILE
    </table>
...
aws s3 cp $OUTPUT_FILE s3://$BUCKET_NAME/ --region $REGION

# Cleanup
rm -rf $BACKUP_DIR $ARCHIVE_FILE

echo "Backup completed and index.html generated with custom logo and professional styling."
